...
```sh
make docker-build docker-push IMG=<some-registry>/kubedeck:tag
```
...
```sh
make deploy IMG=<some-registry>/kubedeck:tag
```
...
**Delete the APIs(CRDs) from the cluster:** 2

```sh
make uninstall
```

**UnDeploy the controller from the cluster:** 1
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/kubedeck/<tag or branch>/dist/install.yaml
```
